...
const FileManager = ({ onOpenFile, initialPath = '/home/arsham' }: FileManagerProps) => {
  ...
  const sidebarItems = [
    { icon: <Home className="w-4 h-4" />, label: 'Home', path: '/home/arsham' },
    { icon: <Star className="w-4 h-4 text-amber-400" />, label: 'Favorites', path: '/home/arsham/projects' },
    { icon: <Clock className="w-4 h-4 text-blue-400" />, label: 'Recent', path: '/home/arsham' },
  ];

  const locations = [
    { label: 'Skills', path: '/home/arsham/skills' },
    { label: 'Projects', path: '/home/arsham/projects' },
  ];

  const getCurrentFolderName = () => {
    if (currentPath === '/home/arsham') return 'Home';
          ...
          <div className="flex items-center gap-1 flex-1 min-w-0 text-[13px]">
            {currentPath.split('/').filter(p => p).map((part, index, arr) => (
              <div key={index} className="flex items-center min-w-0">
                ...
                >
                  {part === 'home' ? '🏠' : part === 'arsham' ? 'Home' : part}
                </button>
              </div>
            ))}
          </div>
